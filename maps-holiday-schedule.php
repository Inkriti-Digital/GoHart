<!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8">
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<title>GoHart</title>
		<meta name="description" content="">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link rel="stylesheet" href="css/styles.min.css">
		<link rel="stylesheet" href="css/styles2.css">
	</head>
	<body>

		<!-- Content -->

		<div class="container alerts-detours holiday-sched">
			<?php include ("views/main-nav.php"); ?>
			
			<section class="header">
				<div class="logo"><img src="img/hart-logo.png"/></div>
				<div class="menu-ico"><img src="img/menu-ico.png"/></div>

				<div class="headline">
					<h1>HOLIDAY &amp;</h1>
					<h1>SCHEDULE</h1>
				</div>
			</section>
			<section class="page-content planner">
				<div class="content-block content-body">
					<div class="bottom-rule">
						<div class="cta"><span>HART HOLIDAY SERVICE SCHEDULE FOR FISCAL YEAR 2015</span><hr/></div>
					</div>
					<div class="info">
						
							<p>Below are the dates of holidays for HART bus, streetcar and paratransit operations schedules, and the general levels of service for each holiday. Saturday service level means services that normally do not operate on Saturdays, such as Commuter Express, will not operate on that holiday. For specific schedule times, please contact the HARTinfo Line at <b>(813) 254-HART.</b></p>

							<p>HART fiscal year 2015 runs October 1, 2014 – September 30, 2015. Special Note for Paratransit Subscription Customers:  Effective September 5, 2012, subscription trips no longer operate during certain holidays. Click here for a list of the holidays.</p>
						
					</div>
					<div class="info-graphic">
						<img src="img/service/info-graphic.jpg"/>
						<br/>
						<span class="paid-ad-text">Paid Ad</span>
					</div>

					<div class="half-column">
						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>VETERAN’S DAY</h2>
								Bus Service Level:  Saturday<br/>
								Streetcar Service Level:  Normal<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>THANKSGIVING DAY</h2>
								Bus Service Level:   Sunday<br/>
								Streetcar Service Level:    No Service<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>DAY AFTER THANKSGIVING</h2>
								Bus Service Level:   Saturday<br/>
								Streetcar Service Level:    Normal<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>CHRISTMAS EVE</h2>
								Bus Service Level:   Saturday<br/>
								Streetcar Service Level:    Wednesday<br/>
								Paratransit Service Level:  As required
							</div>
						</div>
					</div>

					<div class="half-column">
						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>NEW YEAR’S DAY</h2>
								Bus Service Level:  Saturday<br/>
								Streetcar Service Level:  Normal<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>MLK JR. DAY</h2>
								Bus Service Level:   Sunday<br/>
								Streetcar Service Level:    No Service<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>MEMORIAL DAY</h2>
								Bus Service Level:   Saturday<br/>
								Streetcar Service Level:    Normal<br/>
								Paratransit Service Level:  As required
							</div>
						</div>

						<div class="holiday-block">	
							<div class="holiday-date">
								<div class="holiday">11-14<br/>
								2015</div>
							</div>
							<div class="holiday-detail">
								<h2>INDEPENDENCE DAY</h2>
								Bus Service Level:   Saturday<br/>
								Streetcar Service Level:    Wednesday<br/>
								Paratransit Service Level:  As required
							</div>
						</div>
					</div>




					<div class="bottom-rule footer-rule">
						<div class="cta center-cta">
							<div class="callout">
								<div>
									<span class="cta-more">WANT TO FIND OUT MORE? VISIT</span>
									<span class="cta-page"><a href="#">EMERGENCY EVACUATION MAPS</a></span>
								</div>
							</div>
							<hr/>
						</div>
					</div>



					<div class="center footer-ad">
							<img src="img/service/fpo-ad.jpg"/>
							<div class="gototop">
								<img src="img/gotop.png" />
							</div>
						</div>
					</div>
				</div>				
			</section>
		</div>

		<!-- Load Scripts -->
		<?php include ("views/scripts-load.php"); ?>

	</body>
</html>